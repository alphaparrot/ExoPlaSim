...
!
      ...
      real    :: albland  = 0.22  ! albedo for land
      real    :: alblandmax = 0.4
      real    :: alblandmin = 0.069
      
                   ! Non-spectral versions
      real    :: albsmin(NLIGHTS) = 0.4   ! min. albedo for snow
      real    :: albsmax(NLIGHTS) = 0.8   ! max. albedo for snow
      real    :: albsminf(NLIGHTS) = 0.3   ! min. albedo for snow (with forest)
      real    :: albsmaxf(NLIGHTS) = 0.4   ! max. albedo for snow (with forest)
      real    :: albgmin(NLIGHTS) = 0.6   ! min. albedo for glaciers
      real    :: albgmax(NLIGHTS) = 0.8   ! max. albedo for glaciers
                   ! lambda < 0.75 microns
      real    :: albsmin1(NLIGHTS)  = 0.4   ! min. albedo for snow
      real    :: albsmax1(NLIGHTS)  = 0.8   ! max. albedo for snow
      real    :: albsminf1(NLIGHTS) = 0.3   ! min. albedo for snow (with forest)
      real    :: albsmaxf1(NLIGHTS) = 0.4   ! max. albedo for snow (with forest)
      real    :: albgmin1(NLIGHTS)  = 0.6   ! min. albedo for glaciers
      real    :: albgmax1(NLIGHTS)  = 0.8   ! max. albedo for glaciers
                   ! lambda > 0.75 microns
      real    :: albsmin2(NLIGHTS)  = 0.4   ! min. albedo for snow
      real    :: albsmax2(NLIGHTS)  = 0.8   ! max. albedo for snow
      real    :: albsminf2(NLIGHTS) = 0.3   ! min. albedo for snow (with forest)
      real    :: albsmaxf2(NLIGHTS) = 0.4   ! max. albedo for snow (with forest)
      real    :: albgmin2(NLIGHTS)  = 0.6   ! min. albedo for glaciers
      real    :: albgmax2(NLIGHTS)  = 0.8   ! max. albedo for glaciers
...
!
      ...
      real :: dalbclim(NHOR,NLIGHTS)    =  2.0  ! climatological background albedo
      real :: dalbclim1(NHOR,NLIGHTS)   =  2.0  ! climatological background albedo (<.75 um)
      real :: dalbclim2(NHOR,NLIGHTS)   =  2.0  ! climatological background albedo (>.75 um)
!
      end module landmod
...
!       endif
      ...
      if (mypid == NROOT) then
         do klight=1,NLIGHTs
            ...
            albsmaxf2(klight) = 0.5*albsmax2(klight)
            albsmin1(klight) = dsnowalbmn(2*klight-1)
            albsmin2(klight) = dsnowalbmn(2*klight)
            albgmin1(klight) = dglacalbmn(2*klight-1)
            albgmin2(klight) = dglacalbmn(2*klight)
            albsminf1(klight) = 0.75*albsmaxf1(klight)
            albsminf2(klight) = 0.75*albsmaxf2(klight)
         enddo
      endif
      
      
      call mpbcrn(albsmin ,NLIGHTS)
      call mpbcrn(albsmax ,NLIGHTS)
      call mpbcrn(albsminf,NLIGHTS)
      call mpbcrn(albsmaxf,NLIGHTS)
      call mpbcrn(albgmin ,NLIGHTS)
      call mpbcrn(albgmax ,NLIGHTS)
      
      call mpbcrn(albsmin1,NLIGHTS)
      call mpbcrn(albsmax1,NLIGHTS)
      call mpbcrn(albsminf1,NLIGHTS)
      call mpbcrn(albsmaxf1,NLIGHTS)
      call mpbcrn(albgmin1,NLIGHTS)
      call mpbcrn(albgmax1,NLIGHTS)
      
      call mpbcrn(albsmin2,NLIGHTS)
      call mpbcrn(albsmax2,NLIGHTS)
      call mpbcrn(albsminf2,NLIGHTS)
      call mpbcrn(albsmaxf2,NLIGHTS)
      call mpbcrn(albgmin2,NLIGHTS)
      call mpbcrn(albgmax2,NLIGHTS)
...
!          
     ...
     &          +(zalbsnow-dalbclim(jhor,klight))*dsnow(jhor)/(dsnow(jhor)+0.01)
            !Two-band computation
            zalbmax1=dforest(jhor)*albsmaxf1(klight) + (1.-dforest(jhor))*albsmax1(klight)
            zalbmin1=dforest(jhor)*albsminf1(klight) + (1.-dforest(jhor))*albsmin1(klight)
            zdalb1=(zalbmax1-zalbmin1)*(dts(jhor)-263.16)/(tmelt-263.16)
            zalbsnow1=MAX(zalbmin1,MIN(zalbmax1,zalbmax1-zdalb1))
            zalbmax2=dforest(jhor)*albsmaxf2(klight) + (1.-dforest(jhor))*albsmax2(klight)
            zalbmin2=dforest(jhor)*albsminf2(klight) + (1.-dforest(jhor))*albsmin2(klight)
            zdalb2=(zalbmax2-zalbmin2)*(dts(jhor)-263.16)/(tmelt-263.16)
            zalbsnow2=MAX(zalbmin2,MIN(zalbmax2,zalbmax2-zdalb2))
            dsalb(jhor,2*klight-1) = dalbclim1(jhor,klight)                               &
     &          +(zalbsnow1-dalbclim1(jhor,klight))*dsnow(jhor)/(dsnow(jhor)+0.01)
            dsalb(jhor,2*klight  ) = dalbclim2(jhor,klight)                               &
     &          +(zalbsnow2-dalbclim2(jhor,klight))*dsnow(jhor)/(dsnow(jhor)+0.01)
           else
            dalb(jhor,klight)=dalbclim(jhor,klight)
            dsalb(jhor,2*klight-1)=dalbclim1(jhor,klight)
            dsalb(jhor,2*klight  )=dalbclim2(jhor,klight)
...
!         
           ...
           if(dglac(jhor) > 0.5) then
            ...
            dsalb(jhor,2*klight  )=MAX(albgmin2(klight),MIN(albgmax2(klight),albgmax2(klight)-zdalb2))
           end if
          endif
         enddo
       enddo
             ...
             if (dls(jhor)>0.5) then
               if (dts(jhor)>273.15) then !We have wet soil. The wetter, the darker.
                 ...
                 al = 1.0/aa*(max(bf,1.0e-3)**((1.0-yy)/yy) - 1)**(1.0/yy)
                    dalbclim(jhor,klight) = alblandmax * exp(-(bf*25)**6)+ &
&                              al * (1 - exp(-(bf*25)**6) - exp(-((1-bf)*30)**9))+ &
&                              alblandmin * exp(-((1-bf)*30)**9)
                 dalbclim1(jhor,klight) = dalbclim(jhor,klight)
                 dalbclim2(jhor,klight) = dalbclim(jhor,klight)
               else !Frozen soil.
                 dalbclim(jhor,klight) = albland
                 dalbclim1(jhor,klight) = dgroundalb(2*klight-1)
                 dalbclim2(jhor,klight) = dgroundalb(2*klight)
               endif
             endif
           enddo
        enddo
